    ...
    if (level_trigger_unsupported) {
        ...
        if (x86ms->apic_xrupt_override) {
            build_xrupt_override(table_data, 0, 2,
                1 | (1 << 2) /* active high, edge triggered */);
        }
        ...
        if (x86ms->ioapic2) {
            for (i = 0; i < 16; i++) {
                build_ioapic(table_data, IO_APIC_SECONDARY_IRQBASE + i,
        ...
        for (i = 1; i < 16; i++) {
            ...
            build_xrupt_override(table_data, i, i, 0xd /* Flags: Active high, Level Triggered */);
        }
    }
